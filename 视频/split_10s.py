...
if not os.path.exists(log_dir):
    os.makedirs(log_dir)
logging.basicConfig(filename=os.path.join(log_dir, 'split_10s.log'), level=logging.INFO,
                    format='%(asctime)s %(levelname)s:%(message)s')

# root_path = r'E:\split_4\8.15'
# output_root = r'E:\split_12s\8.15' 
root_path = r'D:\文档\深度\m_1080'
output_root = r'D:\文档\深度\1'
...
def split_video(video_file):
    ...
    if duration < 10:
        logging.info(f"视频 {video_file} 时长小于10秒，跳过处理。")
        print(f"视频 {video_file} 时长小于10秒，跳过处理。")
        return
    ...
    if duration < 20:
        # 如果视频时长小于20秒，直接复制原视频
        destination_file = os.path.join(output_dir, f'{base_filename}_original.mp4')
        shutil.copy(video_file, destination_file)
        logging.info(f"视频 {video_file} 时长小于20秒，已复制为 {destination_file}")
        print(f"视频 {video_file} 时长小于20秒，已复制为 {destination_file}")
        return
    ...
    for i in range(num_segments):
        ...
        if end_segment_time - start_segment_time >= 10:  # 确保片段时长至少为10秒
            segments.append((start_segment_time, end_segment_time))

            # 检查倒数第二段是否小于20秒，如果是，则将最后两段合并
    if len(segments) > 1 and segments[-2][1] - segments[-2][0] < 20:
        segments[-2] = (segments[-2][0], segments[-1][1])  # 合并最后两段
        segments.pop()  # 移除原最后一段

    for start_segment_time, end_segment_time in segments:
        output_file = os.path.join(output_dir,
                                   f'{base_filename}_segment_{segments.index((start_segment_time, end_segment_time)) + 1}.mp4')
